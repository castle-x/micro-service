...
- 当前进度以 `.phase/STATUS.md` 为准；当前 AI 协作规范以根目录 `AGENTS.md` 和 `.axm/` 为准。
...
| 优先级 | 文件/目录 | 用途 |
|---|---|---|
| 1 | `.phase/STATUS.md` | 当前阶段、模块完成度、下一阶段路线图的唯一事实源 |
| 2 | `.phase/phase-*.md` | 各阶段关键决策、完成项、延后项和验证结果 |
| 3 | `AGENTS.md` + `.axm/` | AI 协作入口、项目架构规范、编码规范、知识索引 |
| 4 | 实际源码与测试 | 实现细节的最终事实源 |
| 5 | 本文件 | 初始设计意图的历史参考 |
| 6 | 根目录 `SPEC.md` / `初步设计参考.md` | 仅作为路由入口，不承载具体规范 |

若任意历史内容与 `.phase/STATUS.md`、`.axm/` 或源码冲突，以更高优先级为准。
...
| 旧信息 | 当前事实 |
|---|---|
| `SPEC.md` 是最高优先级约束 | 当前以 `AGENTS.md` + `.axm/` 为 AI 规范入口，以 `.phase/STATUS.md` 为进度事实源 |
...
| 旧文档中的长代码模板 | 仅作历史参考，实际写法以当前源码和 `.axm/project/coding.mdc` 为准 |
...
| 旧文档内容 | 当前应读取 |
|---|---|
| 当前阶段 / 下一步 | `.phase/STATUS.md` |
| Phase 01 产物 | `.phase/phase-01-pkg-infra.md` |
| Phase 02 产物 | `.phase/phase-02-pkg-business-skeleton.md` |
| 架构和包边界 | `.axm/project/architecture.mdc` |
| 编码、测试、lint、module 规则 | `.axm/project/coding.mdc` |
| pkg 基础设施事实 | `.axm/knowledge/pkg-infra/overview.mdc` |
| 服务拓扑事实 | `.axm/knowledge/services/overview.mdc` |
| AI 协作入口 | `AGENTS.md` |
...
2. 阶段进度变化只更新 `.phase/STATUS.md` 和对应 `phase-*.md`。
3. 稳态架构/编码规范变化更新 `.axm/project/*.mdc`。
4. 代码事实变化更新 `.axm/knowledge/**/*.mdc`。
