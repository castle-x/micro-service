...
**Phase 03 · idp + iam + Google 登录端到端** — ✅ 已完成
详见 [`phase-03-idp-iam-google-auth.md`](./phase-03-idp-iam-google-auth.md)

## 事实源说明

- 当前进度、路线图和模块状态以本文件为准。
- 各阶段细节以对应 `.phase/phase-*.md` 为准。
- 根目录 `SPEC.md` 与 `初步设计参考.md` 已降级为历史入口；旧设计意图已融合到 [`phase-00-initial-design-reference.md`](./phase-00-initial-design-reference.md)。
...
| `pkg/middleware` | ✅ 完成（P02） | Kitex + Hertz 两套 trace/recovery/logging |
| `pkg/registry` | 🔶 L1 骨架（P02） | etcd 占位，Phase 04 升级 |
| `pkg/mq` | 🔶 L1 骨架（P02） | NSQ 占位，Phase 05 升级 |

### services

| 服务 | 状态 | 说明 |
|------|------|------|
| `services/edge-api` | ✅ 完成（P03） | Hertz HTTP 门面，/auth/google/* 路由，调 idp Kitex RPC |
| `services/idp` | ✅ 完成（P03） | Google OAuth2、identities/oauth_states、JWT 签发/刷新/校验 |
| `services/iam` | ✅ 完成（P03） | User 主数据、UpsertUserByProvider、GetUser、MongoDB 索引 |
...
```
make test   # pkg(14包) + idp(biz/cache) + iam(biz/dal) 全绿
make build  # edge-api / idp / iam 编译通过
```
