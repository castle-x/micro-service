...
`services/` 是业务进程集合：`edge-api` 是公网 HTTP/WebSocket 的 Hertz 门面，`model` 是独立 Hertz HTTP/SSE 模型服务，`asset`、`idp`、`iam` 等业务服务以 Kitex RPC 为主。服务间通信应通过 IDL 生成的 RPC client、HTTP 契约或 MQ 事件完成，禁止直接 import 对方内部业务包。
...
| model | `services/model/` | Hertz | 模型供应商配置、非流式/流式 chat、LLM provider 适配 |
...
- `edge-api`、`idp`、`iam`、`asset`、`model` 已具备可运行入口，并在启动时初始化 `pkg/otel`。
- `idp`、`iam`、`asset` 通过 `pkg/cloudwego` 注册 Kitex 服务到 etcd；`edge-api` 通过 etcd resolver 调用 `idp` / `iam` / `asset`，`idp` 通过 resolver 调用 `iam`。
- `edge-api` 和 `model` 作为 Hertz 服务注册到 etcd；`edge-api -> model` 已使用 `HertzServiceResolver` 解析服务地址后再走现有 HTTP/SSE proxy。
...
edge-api/Hertz → HTTP proxy → model/Hertz
...
- `edge-api` 不直接访问 MongoDB/Redis 业务主存储，业务数据仍通过 idp/iam/asset/model 等后端服务读取或修改。
