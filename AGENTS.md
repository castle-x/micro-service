...
- `idl/` — 全局接口契约目录，Thrift IDL 定义跨服务 RPC 契约和 `base.thrift` 共享结构；`idl/llm/openapi.yaml` 定义 llm HTTP/SSE 契约。
...
- `services/llm/` — Hertz HTTP LLM 服务，负责供应商、模型、密钥管理、非流式 Generate 与 SSE Stream。
- `deployments/` — Docker、Kong、K8s 等运行与部署配置。
- `.axm/progress/` — 阶段进度、路线图、spec 与验收状态记录；替代历史 `.phase/` 目录。

### 依赖方向

`services/* → pkg`；`services/* → idl/generated code`；`edge-api → idp/iam/asset Kitex RPC client`；`edge-api → llm service (HTTP proxy)`；`billing → MQ event → credits/notification`；`pkg` 不依赖任何业务服务。

### 核心约束

- `pkg/` 禁止 import `services/*`。
- `services/*` 之间禁止直接 import 对方内部 Go 包；跨服务通信必须走 IDL + Kitex RPC 或 MQ。
- **例外**：`services/llm/` 使用 Hertz HTTP（而非 Kitex RPC），因为 SSE 流式输出天然属于 HTTP 协议；`edge-api` 通过 HTTP proxy 调用，接口契约见 `idl/llm/openapi.yaml`。
...
| AI 创作平台 / Eino 编排 / 工作流与生图服务设计 | `.axm/progress/generation-platform/roadmap.md` + `.axm/progress/generation-platform/decisions.md` |
| llm service / LLM 适配器 | `idl/llm/openapi.yaml` + `services/llm/component/factory.go` |
